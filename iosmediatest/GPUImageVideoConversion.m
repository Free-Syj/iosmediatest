#import "GPUImageVideoConversion.h"
...
@interface GPUImageVideoConversion () 
{
...
@implementation GPUImageVideoConversion
...
// 不判断Camera支持 FullRange VideoRange 或是 使用 BGRA
    /*
    ...
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        if (captureAsYUV)
        {
            ...
            {
                // glBindAttribLocation 绑定属性  内部与 数组的下标数字进行了绑定 要在 link 前
                [yuvConversionProgram addAttribute:@"position"]; // attribute vec4 position;
                [yuvConversionProgram addAttribute:@"inputTextureCoordinate"]; // attribute vec4 inputTextureCoordinate;
                
                if (![yuvConversionProgram link])
                {// 失败了
                    NSString *progLog = [yuvConversionProgram programLog]; // glGetProgramInfoLog 获取
            ...
            yuvConversionPositionAttribute = [yuvConversionProgram attributeIndex:@"position"]; // 获取属性绑定的ID 就是内部数组的下标
            yuvConversionTextureCoordinateAttribute = [yuvConversionProgram attributeIndex:@"inputTextureCoordinate"];
            yuvConversionLuminanceTextureUniform = [yuvConversionProgram uniformIndex:@"luminanceTexture"]; // 定位uniform变量的地址glGetUniformLocation   shader: uniform sampler2D luminanceTexture;
            yuvConversionChrominanceTextureUniform = [yuvConversionProgram uniformIndex:@"chrominanceTexture"];// uniform sampler2D chrominanceTexture;
            yuvConversionMatrixUniform = [yuvConversionProgram uniformIndex:@"colorConversionMatrix"];// uniform mediump mat3 colorConversionMatrix;    3x3矩阵
            
            [GPUImageContext setActiveShaderProgram:yuvConversionProgram]; // 激活使用 glUseProgram
            
            glEnableVertexAttribArray(yuvConversionPositionAttribute); // 开启顶点数组
...
- (BOOL)isBackFacingCameraPresent
{
    return [GPUImageVideoConversion isBackFacingCameraPresent];
}
...
- (BOOL)isFrontFacingCameraPresent
{
    return [GPUImageVideoConversion isFrontFacingCameraPresent];
}
...
// 更新target  并不一定是变更参数  而是每次传递数据要走的函数
- (void)updateTargetsForVideoCameraUsingCacheTextureAtWidth:(int)bufferWidth height:(int)bufferHeight time:(CMTime)currentTime;
{
    ...
    for (id<GPUImageInput> currentTarget in targets)
    {
        if ([currentTarget enabled])
        {
            ...
            if (currentTarget != self.targetToIgnoreForUpdates) // targetToIgnoreForUpdates 设置了忽略更新
            {
                [currentTarget setInputRotation:outputRotation atIndex:textureIndexOfTarget]; // 设置输出的纹理的方向
                [currentTarget setInputSize:CGSizeMake(bufferWidth, bufferHeight) atIndex:textureIndexOfTarget]; // 设置输出纹理的Size
                
                if ([currentTarget wantsMonochromeInput] && captureAsYUV)
                {
                    [currentTarget setCurrentlyReceivingMonochromeInput:YES];
                    // TODO: Replace optimization for monochrome output
                    [currentTarget setInputFramebuffer:outputFramebuffer atIndex:textureIndexOfTarget];// 设置输出的buffer
                }
                else
                {
...
- (void)updateOrientationSendToTargets;
{
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        {
            outputRotation = kGPUImageNoRotation;
            if (mcameraPosition == AVCaptureDevicePositionBack)
            {
